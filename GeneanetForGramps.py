...
class GeneanetForGrampsOptions(MenuToolOptions):
    ...
    def add_menu_options(self, menu):
        ...
        if verbosity >= 3:
            print(_("Before URL"))
        self.__gui_url = StringOption(
            _("Geneanet URL for the selected person"), ROOTURL)
        ...
        self.__gui_asc.set_help(
            _("Import ascendants of the selected person up to level number"))
        ...
        self.__gui_dsc.set_help(
            _("Import descendants of the selected person up to level number"))
        ...
        self.__gui_verb.set_help(
            _("Verbosity level from 0 (minimal) to 3 (very verbose)"))
...
class GeneanetForGramps(PluginWindows.ToolManagedWindowBatch):
    ...
    def __get_menu_options(self):
        ...
        ascendants = self.options.menu.get_option_by_name(
            'gui_asc').get_value()
        if verbosity >= 3:
            if ascendants:
                print(_("ASC True"))
                ...
                print(_("ASC False"))
        descendants = self.options.menu.get_option_by_name(
            'gui_dsc').get_value()
        ...
        verbosity = self.options.menu.get_option_by_name(
            'gui_verb').get_value()
        if verbosity >= 3:
            print(_("LVL:"), LEVEL)
        save_config()
    ...
    def get_or_create_place(self, event, placename):
        ...
        if pl:
            ...
            for handle in db.get_place_handles():
                ...
                if verbosity >= 4:
                    print(_("DEBUG: search for ") +
                          str(placename) + _(" in ") + str(explace))
    ...
    def get_or_create_event(self, gobj, attr, tran):
        ...
        if self.__dict__[attr + 'date'] \
            ...
            if verbosity >= 2 and self.__dict__[attr + 'date']:
                print(_("Update ") + attr + _(" Date to ") +
                      self.__dict__[attr + 'date'])
...
class GFamily(GBase):
    ...
    def from_geneanet(self):
        ...
        for sr in self.father.spouseref:
            if verbosity >= 3:
                print(_("Comparing sr %s to %s (idx: %d)") %
                      (sr, self.mother.url, idx))
    ...
    def from_gramps(self, gid):
        ...
        try:
            ...
            if verbosity >= 1:
                print(
                    _("WARNING: Unable to retrieve id %s from the gramps db %s") % (gid, gname))
        ...
        if not found:
            ...
            if self.family:
                if verbosity >= 2:
                    print(_("Found an existing Gramps family ") +
                          self.family.gramps_id)
    ...
    def add_child(self, child):
        ...
        for cr in self.family.get_child_ref_list():
            ...
            if c.gramps_id == child.gid:
                found = child
                if verbosity >= 1:
                    print(_("Child already existing : ") +
        ...
        if not found:
            if child:
                if verbosity >= 2:
                    print(_("Adding child: ") +
                          child.firstname + " " + child.lastname)
                childref = ChildRef()
                if child.grampsp:
                    try:
                        childref.set_reference_handle(
                            child.grampsp.get_handle())
                    except:
                        if verbosity >= 2:
                    ...
                    with DbTxn("Geneanet import", db) as tran:
                        db.commit_family(self.family, tran)
                        child.grampsp.add_parent_family_handle(
    ...
    def recurse_children(self, level):
        ...
        if level <= LEVEL and (cpt > 0):
            ...
            if not self.family:
                print(
                    _("WARNING: No family found whereas there should be one :-("))
                return
...
class GPerson(GBase):
    ...
    def from_geneanet(self, purl):
        ...
        try:
            ...
            try:
                ...
                sex = tree.xpath(
                    '//div[@id="person-title"]//img/attribute::alt')
                self.g_sex = sex[0]
            ...
            try:
                name = tree.xpath(
                    '//span[@class="gw-individual-info-name-firstname"]//a/text()')
                ...
                name = tree.xpath(
                    '//span[@class="gw-individual-info-name-lastname"]//a/text()')
                ...
                self.g_lastname = ""
            if verbosity >= 1:
                print(_("==> GENEANET Name (L%d): %s %s") %
                      (self.level, self.g_firstname, self.g_lastname))
            if verbosity >= 2:
            ...
            try:
                ...
                if not match:
                    ...
                    if verbosity >= 2:
                        print(_("Birth place code:"),
                              self.g_birthplacecode)
            except:
                self.g_birthplacecode = None
            try:
            ...
            try:
                ...
                if not match:
                    ...
                    if verbosity >= 2:
                        print(_("Death place code:"),
                              self.g_deathplacecode)
            except:
                self.g_deathplacecode = None
            ...
            for spouse in spouses:
                ...
                try:
                    self.marriageplace.append(
                        str(marriage[s].split(',')[1][1:]).title())
                    if verbosity >= 2:
                ...
                try:
                    ...
                    if not match:
                        ...
                        if verbosity >= 2:
                            print(_("Married place code:"),
                                  self.marriageplacecode[s])
                    ...
                    for a in c.xpath('a'):
                        ...
                        if sosa is None:
                            try:
                                ...
                                if verbosity >= 2:
                                    print(_("Child %d name: %s") %
                                          (cnum, cname))
                            except:
                                cname = ""
                            try:
                                cref = ROOTURL + \
                                    str(a.xpath('attribute::href')[0])
                                if verbosity >= 2:
                                    print(_("Child %d ref: %s") %
                                          (cnum, cref))
                            except:
                                cref = None
    ...
    def find_grampsp(self):
        ...
        for i in ids:
            ...
            dd = format_year(dd)
            if verbosity >= 3:
                print(_("DEBUG: firstname: ") + firstname +
                      _(" vs g_firstname: ") + self.g_firstname)
                print(_("DEBUG: lastname: ") + lastname +
                      _(" vs g_lastname: ") + self.g_lastname)
                if not bd:
                    pbd = "None"
                else:
                    pbd = bd
    ...
    def from_gramps(self, gid):
        ...
        if verbosity >= 2:
            print(_("===> Gramps Name of %s: %s %s") %
                  (self.gid, self.firstname, self.lastname))
        ...
        try:
            ...
            if verbosity >= 1:
                print(_("WARNING: Unable to retrieve birth date for id %s") %
                      (self.gid))
        ...
        try:
            ...
            if verbosity >= 1:
                print(_("WARNING: Unable to retrieve death date for id %s") %
                      (self.gid))
        ...
        while i < len(self.spouseref):
            ...
            for s in self.spouse:
                if s.url == self.spouseref[i]:
                    ...
                    break
            if not spouse:
                spouse = geneanet_to_gramps(
                    None, level, None, self.spouseref[i])
                if spouse:
    ...
    def recurse_parents(self, level):
        ...
        if level <= LEVEL and (self.fref != "" or self.mref != ""):
            ...
            if self.father:
                geneanet_to_gramps(self.father, level,
                                   self.father.gid, self.fref)
                if self.mother:
            ...
            if self.mother:
                geneanet_to_gramps(self.mother, level,
                                   self.mother.gid, self.mref)
                if self.father:
            ...
            # Create a GFamily with them and do a Geaneanet to Gramps for it
            if verbosity >= 2:
                print(_("=> Initialize Parents Family of ") +
...
def geneanet_to_gramps(p, level, gid, url):
    ...
    if gid != None:
        ...
        if p.birthdate == p.g_birthdate or p.deathdate == p.g_deathdate or force:
            ...
            print(_("Gramps   person birth/death: %s / %s") %
                  (p.birthdate, p.deathdate))
            print(_("Geneanet person birth/death: %s / %s") %
                  (p.g_birthdate, p.g_deathdate))
            if not GUIMODE:
                db.close()
                sys.exit(_("Do not continue without force"))
            else:
    ...
    p.to_gramps()
    if GUIMODE:
        progress.set_header(_("Adding Gramps Person %s %s (%s | %s)") % (
